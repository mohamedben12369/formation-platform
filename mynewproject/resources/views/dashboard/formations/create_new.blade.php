        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('dashboard.formations.store') }}" enctype="multipart/form-data">
                        @csrf
                                ...
                                @foreach($themeFormations as $theme)
                                    <div class="form-check">
                                        <input class="form-check-input theme-checkbox" type="checkbox" 
                                               ...
                                               data-prix="{{ $theme->prix ?? 0 }}" 
                                               data-duree="{{ $theme->duree ?? 0 }}"
                                               {{ in_array($theme->idtheme, old('themes', [])) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="theme{{ $theme->idtheme }}">
                                            {{ $theme->titre }} 
                                            <small class="text-muted">({{ $theme->prix ?? 0 }} DH - {{ $theme->duree ?? 0 }}h)</small>
                                        </label>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('prerequis') is-invalid @enderror" 
                                      id="prerequis" name="prerequis" rows="3" 
                                      placeholder="Prérequis nécessaires pour cette formation...">{{ old('prerequis') }}</textarea>
                            @error('prerequis')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
